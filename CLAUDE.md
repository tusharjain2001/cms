...
**The price is ₹999 per website per month, ₹9,990 a year.** A ₹1 test price was run through end to end on 31 Aug 2026 to confirm money actually reaches the bank, and then restored — the ₹1 Razorpay plans (`plan_TWIx2295zBnehi` / `plan_TWIx2EEeoFrklh`) still exist, unused, in case another settlement test is wanted. To reprice, change the two constants in `packages/shared/src/plans.ts` (`99_900` / `999_000` paise) **and** the two in `apps/admin/lib/pricing.ts` (`999` / `9990`), then re-run `npm run setup:razorpay` and swap the new plan ids into `.env`. Every figure the product quotes derives from those four numbers, so that is the whole change — including the tests, which assert against the constant rather than a literal. The price used to be written out in about thirty places and `apps/admin/lib/pricing.ts` exists so it never is again.
