...
**The four policy pages are a payment requirement, not decoration.** A payment provider will not verify a website for payments without Terms, a Privacy Policy, a Refund/Cancellation Policy, a Contact page, and public pricing. Reviewers usually also want a **postal address and phone number** on the Contact page, and an email-only one is the most common single reason a verification bounces.

**Completed 4 Sep 2026, email-only by decision.** The operator is named as "the mypagecraft team", support and privacy mail go to `mypagecraft01@gmail.com`, and **no phone number, postal address, GSTIN or jurisdiction city is published** — the user chose that knowing the review risk above. Nothing is left at the `FILL_ME` sentinel. If Dodo's review sends the site back for a missing address, add lines to `business.address` in `lib/legal.ts` and the Contact page's address block (and its `PostalAddress` structured data) renders again on its own; the Terms fall back to "the courts of India" while `jurisdictionCity` is empty. The phone number was removed outright rather than left blank — there is no `business.phone` field, so it cannot come back by accident.

- **`lib/legal.ts` is the only file you edit to change them.** Operator name, address, emails, GSTIN and jurisdiction live there once; all four pages read from it. Anything set to the `FILL_ME` sentinel renders through `<Fill>` as a loud inline **"TO BE FILLED IN"** marker, so an unfinished policy cannot quietly ship the word `undefined` to the reviewer who decides whether you may take money. Grep the built HTML for that string before submitting.
...
**The wordmark is ahead of the copy.** The logo says `mypagecraft`, and since 4 Sep 2026 so do the four policy pages (`business.tradingName` is `"mypagecraft"`); the page title (`SITE_NAME`), the footer copyright, the docs, the dashboard and the emails still say "Pagecraft". The user has said the product is "mypagecraft everywhere", so the remaining rename is a decided direction — but do it as one deliberate pass over `SITE_NAME`, `lib/og.tsx`, the mailer templates and the dashboard copy, not a blind find-and-replace (the `@pagecraft/*` package names and `PagecraftWordmark` are identifiers, not copy).
